...
var ALL_VECTOR_FEATURES = ee.List(
    IMAGE_COLLECTIONS.values()
).map(function(v){
    return ee.Dictionary(v).get('vector_labels')
}).flatten();

function parse_image_collection(v, range, geometry) {
    v = ee.Dictionary(v);
...
function fetch_feature(f) {
    f = ee.Feature(f);

    var range = ee.DateRange(
        ee.Date(f.get('time_start')).advance(-180, 'day'),
        ee.Date(f.get('time_start'))
    );

    var value_features = image_collections.values().map(function(v){
        return parse_image_collection(v, range, f.geometry());
    }).flatten();

    value_features = ee.FeatureCollection(value_features).sort('time_start');

    var vectors = ALL_VECTOR_FEATURES.map(function(s){
        return value_features.aggregate_array(ee.String(s))
    });
...
exports.fetch = function(fc) {
    return ee.FeatureCollection(fc).map(fetch_feature(fc))
}
